...
import { Send, Sparkles, TrendingUp, AlertTriangle, CheckCircle, BarChart3, Calculator, BookOpen, ChevronRight } from 'lucide-react';
    ...
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'} mb-4`}>
      <div className={`flex gap-2 max-w-[85%] ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
        ...
        <div className={`shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${
          isUser ? 'bg-blue-500' : 'bg-linear-to-br from-purple-500 to-blue-500'
        }`}>
          {isUser ? (
          ...
          {message.insights && message.insights.length > 0 && (
            <div className="mt-3 space-y-2">
              {message.insights.map((insight, idx) => (
                <div key={idx} className="bg-white bg-opacity-20 rounded-lg p-2 text-xs">
                  <div className="font-semibold">{insight.label}</div>
                  <div className="mt-1">{insight.value}</div>
                  {insight.benchmark && (
                    <div className="mt-1 text-opacity-80">Benchmark: {insight.benchmark}</div>
                  )}
                </div>
              ))}
            </div>
          ...
          {message.recommendations && message.recommendations.length > 0 && (
            <div className="mt-3 space-y-2">
              {message.recommendations.map((rec) => (
                <div key={rec.id} className="bg-white bg-opacity-20 rounded-lg p-3 text-xs">
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex-1">
                      <div className="font-semibold">{rec.title}</div>
                      <div className="mt-1 text-opacity-90">{rec.description}</div>
                      {rec.estimatedSavings && (
                        <div className="mt-2 font-semibold">
                          Potential savings: ₦{(rec.estimatedSavings / 1000).toFixed(0)}K/month
                        </div>
                      )}
                    </div>
...
const InsightsWidget: React.FC<{ onInsightClick: (insight: ProactiveInsight) => void }> = ({ onInsightClick }) => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
      <div className="bg-linear-to-r from-purple-500 to-blue-500 px-4 py-3 flex items-center gap-2">
          ...
          <div key={insight.id} className="p-4 hover:bg-gray-50 transition-colors">
            <div className="flex items-start gap-3">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${
                ...
                insight.type === 'opportunity' ? 'bg-yellow-100' : 'bg-green-100'
              }`}>
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <span className={`text-xs font-semibold uppercase ${
                        ...
                        insight.type === 'opportunity' ? 'text-yellow-600' : 'text-green-600'
                      }`}>
                        {insight.type === 'critical' ? '⚠️ Critical' :
                         insight.type === 'opportunity' ? '⭐ Opportunity' : '✅ Best Practice'}
                      </span>
                    </div>
                    <h4 className="font-semibold text-gray-900 mt-1">{insight.title}</h4>
                    <p className="text-sm text-gray-600 mt-1">{insight.description}</p>
                    {insight.metric && (
                      <p className="text-sm font-semibold text-gray-900 mt-2">{insight.metric}</p>
                    )}
                  </div>
                </div>
        ...
        <div className="flex-1 space-y-3">
          <div>
            ...
            <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
              <div className="h-full bg-linear-to-r from-blue-500 to-blue-600" style={{ width: '75%' }} />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
              <div className="h-full bg-linear-to-r from-green-500 to-green-600" style={{ width: '90%' }} />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
              <div className="h-full bg-linear-to-r from-yellow-500 to-yellow-600" style={{ width: '60%' }} />
            </div>
          </div>
        </div>
      </div>
...
const generateRuleBasedResponse = (query: string, financialData: FinancialData): Message => {
  const lowerQuery = query.toLowerCase();
  const timestamp = new Date();
  
  // Cash flow inquiry
  if (lowerQuery.includes('cash flow') || lowerQuery.includes('running out') || lowerQuery.includes('cash')) {
    return {
  ...
  if (lowerQuery.includes('logistics') || lowerQuery.includes('expense') || lowerQuery.includes('cost') || lowerQuery.includes('spending')) {
    return {
      id: Date.now().toString(),
      type: 'ai',
      ...
      recommendations: [
        {
          ...
          description: 'Combine weekly deliveries to reduce number of trips from 12 to 6 per month',
        ...
        },
        {
          ...
          description: 'Your volume qualifies for 15-20% bulk discount with major couriers',
        ...
        },
        {
          ...
          description: 'GIG Logistics and Kwik offer 20% lower rates for your routes',
  ...
  // Hiring inquiry
  if (lowerQuery.includes('hire') || lowerQuery.includes('assistant') || lowerQuery.includes('staff') || lowerQuery.includes('employee')) {
    return {
      id: Date.now().toString(),
      type: 'ai',
  ...
  // Tax inquiry
  if (lowerQuery.includes('tax') || lowerQuery.includes('paye') || lowerQuery.includes('vat')) {
    return {
      id: Date.now().toString(),
      type: 'ai',
...
const AI: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const greeting: Message = {
      ...
      timestamp: new Date(),
      actions: [
        { label: 'Show Insights', type: 'primary', onClick: () => setShowInsights(true) },
        { label: 'Health Score', type: 'secondary', onClick: () => setShowHealthScore(true) }
      ]
    };
  ...
  const handleSendMessage = async () => {
    ...
    setTimeout(() => {
      const aiResponse = generateRuleBasedResponse(inputValue, mockFinancialData);
        ...
        <div className="bg-white rounded-t-2xl shadow-sm border border-gray-200 px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold text-gray-900">FinEdge AI Coach</h1>
                <p className="text-sm text-gray-500">Your personal CFO, available 24/7</p>
              </div>
            </div>
            <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-4">
          ...
          {showInsights && (
            <div className="lg:col-span-1 space-y-4">
              ...
              {showHealthScore && (
                <FinancialHealthScore score={mockFinancialData.cashFlowStability} />
              )}
            </div>
          )}
          
          {/* Chat Area */}
          <div className={`${showInsights ? 'lg:col-span-2' : 'lg:col-span-3'}`}>
              ...
              <div className="border-t border-gray-200 p-4">
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="px-6 py-3 bg-gradient-to-r from-purple-500 to-blue-500 text-white rounded-xl font-medium hover:from-purple-600 hover:to-blue-600 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                  >
